...
#         currency_usd = {"id": 1, "code": "USD", "name": "USD...", "rate": 1.0, "updated_at": datetime.now(UTC)}
#         stmt = insert(currency_rate).values(id=1, code="USD", name="USD...", rate=1)
#         await session.execute(stmt)
#         await session.commit()
#
#         currency_eur = {"id": 2, "code": "EUR", "name": "EUR...", "rate": 0.9246, "updated_at": datetime.now(UTC)}
#         stmt = insert(currency_rate).values(id=2, code="EUR", name="EUR...", rate=0.9246)
#         await session.execute(stmt)
#         await session.commit()
#
